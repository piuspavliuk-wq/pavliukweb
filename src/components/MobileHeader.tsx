...
const navItems = [
  { href: "/", label: "Home", variant: "secondary" as const },
  { href: "/case-study", label: "Case Study", variant: "secondary" as const },
    ...
    <>
      ...
      >
        <button
          ...
          className="-ml-2 flex h-10 w-10 cursor-pointer flex-col items-center justify-center gap-1.5 rounded-lg transition-colors hover:bg-surface focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary [&_*]:cursor-pointer"
          aria-label="Open menu"
          aria-expanded={isOpen}
        >
          <span className="block h-0.5 w-5 rounded-full bg-text" />
          <span className="block h-0.5 w-5 rounded-full bg-text" />
          <span className="block h-0.5 w-5 rounded-full bg-text" />
        </button>
        <Link
          href={homeHref}
          className="text-sm font-semibold text-text transition-colors hover:text-accent md:hidden"
        >
        ...
        {isOpen && (
          <>
            <motion.div
              ...
              className="fixed inset-0 z-[60] cursor-pointer backdrop-blur-sm bg-bg/80"
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 z-[70] flex h-full w-64 flex-col gap-6 border-r border-text-secondary/10 bg-surface/95 px-6 py-8 backdrop-blur-md"
              aria-label="Menu"
            >
              <button
                type="button"
